...
function House() {
  ...
  useEffect(() => {
    const cards = gsap.utils.toArray(".house");
  ...
  return (
    <section id="projects">
      <div className="container">
        <h1 className="text-2xl lg:text-4xl text-[#FFC045]">Hovli - Joy</h1>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 py-5 lg:py-10">
          {data.map((item, i) => (
            <div
              key={i}
              className={`house w-full p-2 md:p-4 border border-[#FFC045] flex flex-col gap-2 shadow-md`}
            >
              <div>
                <Image className="w-auto h-auto" width={250} height={360} src={item.image} alt="" />
              </div>
              <div>
